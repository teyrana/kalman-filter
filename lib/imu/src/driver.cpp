...
int Driver::configure(){
    ...
    {   // set: static running average mode (0x00)
        ...
        IMU::Command<1,0> set_axis_directions_command( 116, 0 );
        ...
        set_axis_directions_command.data(0) = 0x08 | 0x03; 
        set_axis_directions_command.pack();
#endif 
        log.debug("    >> Mapping Axes => {} ...", set_axis_directions_command[2]);
...
bool Driver::stream() {
    ...
    // debug
    stream_interval = 250 * 1000;
    stream_duration = 6 * 1000 * 1000;
